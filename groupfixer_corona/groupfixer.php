...
//---->ych
//argv[1] is the full path to whosin.db
//argv[2] is the full path to EXCEL.xls

$handle = new SQLite3($argv[1]);
...
{	
  ...
  $cars_no = ceil(count($day_teachers)/PASSENGERS_PER_CAR); //cars no per day
...
///////////////////////////////////////////////////dromologia output in HTML//////////////////////////////////
    /*for ($day = 0; $day <= DAYS_PER_WEEK-1; ++$day)//foreach ($dromo_array as $day_dromos) 
    {
      foreach($cars[$day] as $car){
        $car->printout();
        $driver_stats[$car->driver->name]+=1;
        //echo $car->driver->name;
        //echo "<br>";
      }
    echo "<br>";
    }*/
///////////////////////////////////////////////////if isokatanomi checked optimise drivers
if (true){
  for ($day = 0; $day <= DAYS_PER_WEEK-1; ++$day)//foreach ($dromo_array as $day_dromos) 
  {
    foreach($cars[$day] as $car){
  ...
  asort($driver_stats);
  end($driver_stats);         // move the internal pointer to the end of the array
  $last_driver = key($driver_stats);  // fetches the key of the element pointed to by the internal last driver who drives most in sorted array			
  while ( $driver_stats[$last_driver]- reset($driver_stats) >1){ //driference more than 1 in driving timesbetween first and last driver
    foreach ($driver_stats as $driver => $drive_times) { //starts loop in array, first drivers drive less 
      if($driver_stats[$last_driver] - $driver_stats[$driver] >1){ //driference more than 1 in driving times
        for ($day = 0; $day <= DAYS_PER_WEEK-1; ++$day){//loop all days and crs to see if possible to sitch drivers
          foreach($cars[$day] as $car){
            if ($car->switch_driver($last_driver, $driver, $car)){ //if switch driver with
              $driver_stats[$driver]++; // fix stats after switch
              $driver_stats[$last_driver]--;
              if($driver_stats[$last_driver] - $driver_stats[$driver] < 2 ){
                break 2;
              }
            }
          }
    ...
    array_pop($driver_stats); //delete last item
    asort($driver_stats);
    end($driver_stats);         // move the internal pointer to the end of the array
    $last_driver = key($driver_stats);  // fetches the key of the element pointed to by the internal last driver who drives most in sorted array
  }
...
// print driver stats
    /*asort($driver_stats);
    //print_r($driver_stats);
    foreach ($driver_stats as $driver => $drive_times) { 
      echo $driver; 
      echo ": ";
      echo $drive_times;
      echo "<br>";
